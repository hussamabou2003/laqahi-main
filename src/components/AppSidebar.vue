<template>
  <aside class="sidebar-card-container">
    <div style="display: flex; justify-content: space-between; align-items: center; padding: 0 8px 10px;">
      <div class="logo">
        <LogoIcon size="28" />
    ...
    <div class="user-profile">
      ...
      <div class="user-meta">
        <p class="user-name">{{ authStore.currentUser?.fullName || $t('sidebar.parent') }}</p>
        <span class="user-role">{{ $t("sidebar.guardian") }}</span>
      </div>
    </div>

    <nav class="nav-menu">
      <router-link to="/dashboard" class="nav-item">
        <i class="ti ti-layout-dashboard"></i><span>{{ $t("sidebar.dashboard") }}</span>
      </router-link>
      <router-link to="/children" class="nav-item">
        <i class="ti ti-users"></i><span>{{ $t("sidebar.myChildren") }}</span>
      </router-link>
      <button type="button" class="nav-item nav-btn" @click="goToSection('vaccines')">
        <i class="ti ti-vaccine"></i><span>{{ $t("sidebar.vaccines") }}</span>
      </button>
      <button type="button" class="nav-item nav-btn" @click="goToSection('appointments')">
        <i class="ti ti-calendar"></i><span>{{ $t("sidebar.appointments") }}</span>
      </button>
    ...
    <div class="sidebar-bottom">
      <a href="#" class="support-link"><i class="ti ti-help-circle"></i><span>{{ $t("sidebar.support") }}</span></a>
      <button type="button" class="logout-link" @click="handleLogout">
        <i class="ti ti-logout"></i><span>{{ $t("sidebar.logout") }}</span>
      </button>
    </div>
  </aside>
</template>
...
function goToSection(target) {
  uiStore.scrollTarget = target
  if (route.path !== '/dashboard') {
    router.push({ path: '/dashboard' }).then(() => scrollToElement(target))
  } else {
...
.user-profile { display: flex; align-items: center; gap: 12px; padding: 10px 14px; background: var(--color-surface-muted); border-radius: 16px; border: 1px solid var(--color-border); }
